...
int CInn::Stay(CHuman* man) {   //  CHumanクラスのポインタ型変数manに入るCHumanインスタンスのSleep関数を呼び出す。このプログラムでは、CFighterもしくはCMageクラスのインスタンス
    return man->Sleep(1.5);
}

int main() {
    int characterType;
    std::cout << "Choose character type (1 for Fighter, 2 for Mage): ";
